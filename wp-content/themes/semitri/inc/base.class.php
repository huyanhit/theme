...
class ZO_Base
{
    ...
    public static function getPageTitle(){
        ...
        if (!is_archive()){
            ...
            /* search */
            elseif (is_search()):
                printf( __( 'Search Results for: %s', 'semitri' ), '<span>' . get_search_query() . '</span>' );
            /* 404 */ 
            elseif (is_404()):
                _e( 'Page Not Found', 'semitri');
            /* other */
            else :
                the_title();
            endif;
            ...
            /* author. */
            elseif ( is_author() ) :
                printf( __( 'Author: %s', 'semitri' ), '<span class="vcard">' . get_the_author() . '</span>' );
            /* date */
            elseif ( is_day() ) :
                printf( __( 'Day: %s', 'semitri' ), '<span>' . get_the_date() . '</span>' );
            elseif ( is_month() ) :
                printf( __( 'Month: %s', 'semitri' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'semitri' ) ) . '</span>' );
            elseif ( is_year() ) :
                printf( __( 'Year: %s', 'semitri' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'semitri' ) ) . '</span>' );
            /* post format */
            elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
                _e( 'Asides', 'semitri' );
            elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
                _e( 'Galleries', 'semitri');
    ...
    public static function getBreadCrumb($separator = '') {
        ...
        /* search */
        if( is_search() ){ echo '<li>'.__("Search", 'semitri').'</li>'; }
        /* date */
        if( is_year() ){ echo '<li>'.get_the_time('Y').'</li>'; }
        /* 404 */
        if( is_404() ) {
            echo '<li>'.__("404 - Page not Found", 'semitri').'</li>';
        }
        /* archive */
	...
	public static function setGoogleFont($googlefont = array(), $selecter){
		$font_weight = !empty($googlefont['font-weight']) ? 'font-weight:'.$googlefont['font-weight'].';' : '' ;
