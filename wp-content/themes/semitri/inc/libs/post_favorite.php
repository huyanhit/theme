...
class NectarLove {
	...
	function add_love($unit = '',$show_icon = true) {
		...
		$output = $this->love_post($post->ID);
  		...
  		$class = 'nectar-love';
  		$icon = 'fa fa-heart-o';
  		$title = esc_html__('Love this', 'semitri');
		if( isset($_COOKIE['nectar_love_'. $post->ID]) ){
			$class = 'nectar-love loved';
			$icon = 'fa fa-heart';
			$title = esc_html__('You already love this!', 'semitri');
		}
		$heart_icon = '';
		if($show_icon){
			$heart_icon = '<i class="'.$icon.'"></i>';
		}
		return '<a itemscope href="#" class="'. $class .'" itemid="nectar-love-'. $post->ID .'" itemtype="'. get_the_permalink($post->ID) .'" title="'. $title .'">'.$heart_icon . esc_html__(' Likes ','semitri') . $output .' '.$unit.'</a>';
	}
