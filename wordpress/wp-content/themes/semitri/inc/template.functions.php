			 ...
			 <div class="container">
				<div class="row">
					<?php switch ($page_title_layout){
						case '1':
							?>
							<div id="page-title-text" class="col-xs-12 col-sm-12 col-md-12 col-lg-12"><h1><?php $zo_base->getPageTitle(); ?></h1>
							<div class="page-line-top"></div>
							<div class="page-line-bot"></div>
							<?php zo_page_sub_title(); ?></div>
							<div id="breadcrumb-text" class="col-xs-12 col-sm-12 col-md-12 col-lg-12"><?php $zo_base->getBreadCrumb(); ?></div>
							<?php
							break;
						case '2':
							?>
							<div id="breadcrumb-text" class="col-xs-12 col-sm-12 col-md-12 col-lg-12"><?php esc_html_e('You are here: ', 'semitri')?><?php $zo_base->getBreadCrumb(); ?></div>
						...
						case '3':
							...
							<div id="breadcrumb-text" class="col-xs-12 col-sm-6 col-md-6 col-lg-6"><?php esc_html_e('You are here: ', 'semitri')?><?php $zo_base->getBreadCrumb(); ?></div>
							<?php
							break;
						case '4':
							?>
...
function zo_archive_detail(){
    ?>
    <div class="author vcard"><?php _e('By', 'semitri'); ?> <?php the_author_posts_link(); ?></div>
    <ul>
...
function zo_archive_readmore(){
    echo '<a class="btn btn-default" href="'.get_the_permalink().'" title="'.get_the_title().'" >'.__('Continue Reading', 'semitri').'</a>';
}
...
function zo_archive_post_icon() {
    $post_icon = array('icon'=>'fa fa-file-text-o','text'=>__('STANDARD', 'semitri'));
    switch (get_post_format()) {
        case 'gallery':
            $post_icon['icon'] = 'fa fa-camera-retro';
            $post_icon['text'] = __('GALLERY', 'semitri');
            break;
        case 'link':
            $post_icon['icon'] = 'fa fa-external-link';
            $post_icon['text'] = __('LINK', 'semitri');
            break;
        case 'quote':
            $post_icon['icon'] = 'fa fa-quote-left';
            $post_icon['text'] = __('QUOTE', 'semitri');
            break;
        case 'video':
            $post_icon['icon'] = 'fa fa-youtube-play';
            $post_icon['text'] = __('VIDEO', 'semitri');
            break;
        case 'audio':
            $post_icon['icon'] = 'fa fa-volume-up';
            $post_icon['text'] = __('AUDIO', 'semitri');
            break;
        default:
            $post_icon['icon'] = 'fa fa-image';
            $post_icon['text'] = __('STANDARD', 'semitri');
            break;
    }
...
function zo_comment_nav() {
	...
	<nav class="navigation comment-navigation" role="navigation">
		<div class="nav-links">
			<?php
				if ( $prev_link = get_previous_comments_link( __( 'Older Comments', 'semitri' ) ) ) :
					printf( '<div class="nav-previous">%s</div>', $prev_link );
				endif;

				if ( $next_link = get_next_comments_link( __( 'Newer Comments', 'semitri' ) ) ) :
